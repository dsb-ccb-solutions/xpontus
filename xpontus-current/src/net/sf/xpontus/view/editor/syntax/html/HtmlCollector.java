            ...
            if (t.tagName.equalsIgnoreCase(ts.tagName))
              {
                ...
                blockElements = new HtmlDocument.ElementSequence(elements.size() -
                        ts.index - 1);

                for (int j = ts.index + 1; j < elements.size(); j++)
                    blockElements.addElement((HtmlDocument.HtmlElement) elements.elementAt(
                            j));
    ...
    public static void main(String[] args) throws Exception
      {
        ...
        try
          {
            HtmlDocument document = new HtmlParser(new FastCharStream(
