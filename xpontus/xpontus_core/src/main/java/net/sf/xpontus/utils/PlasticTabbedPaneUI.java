...
public class PlasticTabbedPaneUI extends MetalTabbedPaneUI {
    ...
    protected void layoutLabel(int tabPlacement, FontMetrics metrics,
        ...
        if (((tabPlacement == RIGHT) || (tabPlacement == LEFT)) &&
                (icon != null) && (title != null) && !title.equals("")) {
            SwingUtilities.layoutCompoundLabel(tabPane, metrics, title, icon,
                SwingConstants.CENTER, SwingConstants.LEFT,
                SwingConstants.CENTER, SwingConstants.TRAILING, calcRectangle,
    ...
    protected Icon getIconForTab(int tabIndex) {
        ...
        return (!isTabIconsEnabled && hasTitle) ? null
    ...
    protected void paintContentBorder(Graphics g, int tabPlacement,
        ...
        Rectangle selRect;
        selRect = (selectedIndex < 0) ? null
                                      : getTabBounds(selectedIndex, calcRect);

        boolean drawBroken = (selectedIndex >= 0) &&
            isTabInFirstRun(selectedIndex);
        ...
        renderer.paintContentBorderTopEdge(g, x, y, w, h, drawBroken, selRect,
            isContentBorderPainted);
        renderer.paintContentBorderLeftEdge(g, x, y, w, h, drawBroken, selRect,
            isContentBorderPainted);
        renderer.paintContentBorderBottomEdge(g, x, y, w, h, drawBroken,
            selRect, isContentBorderPainted);
        renderer.paintContentBorderRightEdge(g, x, y, w, h, drawBroken,
            selRect, isContentBorderPainted);
    }
    ...
    protected Insets getContentBorderInsets(int tabPlacement) {
        return renderer.getContentBorderInsets(super.getContentBorderInsets(
                tabPlacement));
    }
    ...
    protected boolean shouldPadTabRun(int tabPlacement, int run) {
        return renderer.shouldPadTabRun(run,
            super.shouldPadTabRun(tabPlacement, run));
    }
    ...
    protected void paintFocusIndicator(Graphics g, int tabPlacement,
        ...
        renderer.paintFocusIndicator(g, rectangles, tabIndex, iconRect,
            textRect, isSelected);
    }
        ...
        protected void calculateTabRects(int tabPlacement, int tabCount) {
            ...
            case LEFT:
                ...
                returnAt = size.height -
                    (insets.bottom + theTabAreaInsets.bottom);

                break;

            case RIGHT:
                maxTabWidth = calculateMaxTabWidth(tabPlacement);
                x = size.width - insets.right - theTabAreaInsets.right -
                    maxTabWidth;
                y = insets.top + theTabAreaInsets.top;
                returnAt = size.height -
                    (insets.bottom + theTabAreaInsets.bottom);
            ...
            case BOTTOM:
                ...
                x = insets.left + theTabAreaInsets.left;
                y = size.height - insets.bottom - theTabAreaInsets.bottom -
                    maxTabHeight;
                returnAt = size.width -
                    (insets.right + theTabAreaInsets.right);
            ...
            for (i = 0; i < tabCount; i++) {
                ...
                if (!verticalTabRuns) {
                    ...
                    if ((tabInRun != 0) &&
                        ...
                        runCount++;
                        rect.x = x;
                        runReturnAt = runReturnAt -
                            (2 * getTabRunIndent(tabPlacement, runCount));
                    }
                    ...
                    if ((tabInRun != 0) &&
                            ((rect.y + rect.height) > runReturnAt)) {
                        if (runCount > (tabRuns.length - 1)) {
                            expandTabRunsArray();
                        }
                        ...
                        rect.y = y;
                        tabInRun = 0;
                        runReturnAt -= (2 * getTabRunIndent(tabPlacement,
                            runCount));
                    }
            ...
            for (i = runCount - 1; i >= 0; i--) {
                ...
                if (!verticalTabRuns) {
                    ...
                    if (shouldPadTabRun(tabPlacement, i)) {
                        padTabRun(tabPlacement, start, end,
                            returnAt - (2 * indent));
                    }
    ...
    private class TabbedPaneScrollLayout extends TabbedPaneLayout {
        ...
        public void layoutContainer(Container parent) {
            ...
            if (selectedIndex < 0) {
                ...
                int tx; // tab area bounds
                int ty; // tab area bounds
                int tw; // tab area bounds
                int th; // tab area bounds
                int cx; // content area bounds
                int cy; // content area bounds
                int cw; // content area bounds
                        ...
                        if ((tabScroller != null) &&
                            ...
                            case RIGHT:
                                ...
                                if (totalTabHeight > th) {
                                    ...
                                    vh = (th > (2 * butSize.height))
                                        ? (th - (2 * butSize.height)) : 0;
                            ...
                            case TOP:default:
                                ...
                                if (totalTabWidth > tw) {
                                    ...
                                    vw = (tw > (2 * butSize.width))
                                        ? (tw - (2 * butSize.width)) : 0;
                            ...
                            case RIGHT:
                                ...
                                if (totalTabHeight > th) {
                                    visible = true;
                                    bx = ((tabPlacement == LEFT)
                                        ? ((tx + tw) - bsize.width) : tx);
        ...
        public void stateChanged(ChangeEvent e) {
            ...
            case LEFT:
                ...
                scrollForwardButton.setEnabled((leadingTabIndex < (tabCount -
                    1)) && ((viewSize.height - viewRect.y) > viewRect.height));
            ...
            case BOTTOM:
                ...
                scrollBackwardButton.setEnabled((viewRect.x > 0) &&
                    (leadingTabIndex > 0));
                scrollForwardButton.setEnabled((leadingTabIndex < (tabCount -
                    1)) && ((viewSize.width - viewRect.x) > viewRect.width));
    ...
    private static class ArrowButton extends JButton implements UIResource {
        ...
        private void paintArrow(Graphics g) {
            ...
            Color shadow = UIManager.getColor("ScrollBar.highlight");
    ...
    private abstract static class AbstractRenderer {
        ...
        private static AbstractRenderer createEmbeddedRenderer(
            JTabbedPane tabPane) {
            switch (tabPane.getTabPlacement()) {
            case SwingConstants.TOP:
...
//            selectLight = new Color(((2 * selectColor.getRed()) +
//                    selectHighlight.getRed()) / 3,
//                    ((2 * selectColor.getGreen()) + selectHighlight.getGreen()) / 3,
//                    ((2 * selectColor.getBlue()) + selectHighlight.getBlue()) / 3);
        }

        protected boolean isFirstDisplayedTab(int tabIndex, int position,
            int paneBorder) {
            return tabIndex == 0;
         ...
         * Draws the rectancle around the Tab label which indicates keyboard focus.
         */
        protected abstract void paintFocusIndicator(Graphics g,
         ...
         * tabs is handled correctly.
         */
        protected abstract void paintTabBackground(Graphics g, int tabIndex,
            int x, int y, int w, int h, boolean isSelected);

        /**
         * Paints the border around the given tab.
         */
        protected abstract void paintTabBorder(Graphics g, int tabIndex, int x,
         ...
         */
        protected void paintContentBorderTopEdge(Graphics g, int x, int y,
            int w, int h, boolean drawBroken, Rectangle selRect,
         ...
         */
        protected void paintContentBorderBottomEdge(Graphics g, int x, int y,
            int w, int h, boolean drawBroken, Rectangle selRect,
            boolean isContentBorderPainted) {
            if (isContentBorderPainted) {
         ...
         * and TabPlacement is same as painted edge
         */
        protected void paintContentBorderLeftEdge(Graphics g, int x, int y,
            int w, int h, boolean drawBroken, Rectangle selRect,
            boolean isContentBorderPainted) {
            if (isContentBorderPainted) {
                g.setColor(selectHighlight);
         ...
         * and TabPlacement is same as painted edge
         */
        protected void paintContentBorderRightEdge(Graphics g, int x, int y,
            int w, int h, boolean drawBroken, Rectangle selRect,
            boolean isContentBorderPainted) {
            if (isContentBorderPainted) {
                g.setColor(darkShadow);
    ...
    private static final class BottomEmbeddedRenderer extends AbstractRenderer {
        ...
        protected Insets getTabInsets(int tabIndex, Insets tabInsets) {
            return new Insets(tabInsets.top, tabInsets.left, tabInsets.bottom,
                tabInsets.right);
        }
         ...
         */
        protected void paintFocusIndicator(Graphics g, Rectangle[] rects,
            int tabIndex, Rectangle iconRect, Rectangle textRect,
            boolean isSelected) {
            // Embedded tabs paint no focus.
        }

        protected void paintTabBackground(Graphics g, int tabIndex, int x,
            int y, int w, int h, boolean isSelected) {
            g.setColor(selectColor);
            g.fillRect(x, y, w + 1, h);
        }

        protected void paintTabBorder(Graphics g, int tabIndex, int x, int y,
    ...
    private static final class BottomRenderer extends AbstractRenderer {
        ...
        protected Insets getTabAreaInsets(Insets defaultInsets) {
            return new Insets(defaultInsets.top, defaultInsets.left + 5,
        ...
        protected Insets getTabInsets(int tabIndex, Insets tabInsets) {
            return new Insets(tabInsets.top, tabInsets.left - 2,
                tabInsets.bottom, tabInsets.right - 2);
        }
        ...
        protected void paintContentBorderBottomEdge(Graphics g, int x, int y,
            ...
            if (drawBroken && (selRect.x >= x) && (selRect.x <= (x + w))) {
                ...
                if ((selRect.x + selRect.width) < ((x + w) - 2)) {
                    g.setColor(darkShadow);
                    g.fillRect((selRect.x + selRect.width + 2) - x, 0,
                        right - selRect.x - selRect.width - 2, 1);
                }
            } else {
                g.setColor(darkShadow);
    ...
    private static final class LeftRenderer extends AbstractRenderer {
        ...
        protected Insets getTabAreaInsets(Insets defaultInsets) {
            return new Insets(defaultInsets.top + 4, defaultInsets.left,
                defaultInsets.bottom, defaultInsets.right);
        }
        ...
        protected Insets getTabInsets(int tabIndex, Insets tabInsets) {
            return new Insets(tabInsets.top, tabInsets.left - 5,
                tabInsets.bottom + 1, tabInsets.right - 5);
        }
        ...
        protected void paintContentBorderLeftEdge(Graphics g, int x, int y,
            ...
            if (drawBroken && (selRect.y >= y) && (selRect.y <= (y + h))) {
                ...
                if ((selRect.y + selRect.height) < ((y + h) - 2)) {
                    g.fillRect(x, (selRect.y + selRect.height) - 1, 1,
                        (y + h) - selRect.y - selRect.height);
                }
            } else {
                g.fillRect(x, y, 1, h - 1);
            }
    ...
    private static final class TopEmbeddedRenderer extends AbstractRenderer {
        ...
        protected Insets getTabInsets(int tabIndex, Insets tabInsets) {
            return new Insets(tabInsets.top, tabInsets.left + 1,
    ...
    private static final class TopRenderer extends AbstractRenderer {
        ...
        protected Insets getTabAreaInsets(Insets defaultInsets) {
            return new Insets(defaultInsets.top, defaultInsets.left + 4,
                defaultInsets.bottom, defaultInsets.right);
        }
        ...
        protected Insets getTabInsets(int tabIndex, Insets tabInsets) {
            return new Insets(tabInsets.top - 1, tabInsets.left - 4,
                tabInsets.bottom, tabInsets.right - 4);
        }
        ...
        protected void paintContentBorderTopEdge(Graphics g, int x, int y,
            ...
            if (drawBroken && (selRect.x >= x) && (selRect.x <= (x + w))) {
                ...
                if ((selRect.x + selRect.width) < ((x + w) - 2)) {
                    g.fillRect(selRect.x + selRect.width + 2, top,
                        right - 2 - selRect.x - selRect.width, 1);
                } else {
