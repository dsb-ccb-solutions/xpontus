...
public final class FindReplaceUtility {
    ...
    private static final JDialog FIND_REPLACE_DIALOG = new JDialog((Frame) DefaultXPontusWindowImpl.getInstance()
                                                                                                   .getDisplayComponent(),
            false);
    private static final JPanel TEXT_FIELD_PANEL = new JPanel(new GridLayout(
                2, 1));
    ...
    private static final FocusAdapter TEXT_FOCUS_LISTENER = new FocusAdapter() {
            public void focusGained(FocusEvent fe) {
                ...
                attributeSet = textComponent.getDocument()
    ...
    static {
        ...
        TEXT_FIELD_PANEL.setLayout(new BoxLayout(TEXT_FIELD_PANEL,
                BoxLayout.Y_AXIS));
    ...
    private static void fireTextEvent() {
        ...
        if ((lstrs != null) && (lstrs.length > 0)) {
            TextEvent te = new TextEvent(FIND_REPLACE_DIALOG,
    ...
    private static int findNext(boolean reverse, int pos) {
        ...
        if ((pattern != null) && (pattern.length() > 0)) {
            ...
            char first = backwards ? pattern.charAt(pattern.length() - 1)
                                   : pattern.charAt(0);
            ...
            while (!found && (backwards ? (pos > end) : (pos < end))) {
                found = !MATCH_CASE_CHECKBOX.isSelected() &&
                    (SEGMENT.array[pos] == oppFirst);
    ...
    private static void setListStrings() {
        ...
        if ((findObject != null) && (replaceObject != null)) {
            ...
            if (REPLACE_FIELD.isShowing()) {
                ...
                for (int i = 0; !found && (i < REPLACE_FIELD.getItemCount());
                        i++) {
    ...
    public static void showDialog(boolean isReplace) {
        textComponent = DefaultXPontusWindowImpl.getInstance()
        ...
        Dimension d = isReplace ? REPLACE_ALL_BUTTON.getPreferredSize()
