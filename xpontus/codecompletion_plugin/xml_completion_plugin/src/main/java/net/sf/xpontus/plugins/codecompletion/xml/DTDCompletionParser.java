...
{
    private static final Log LOG = LogFactory.getLog(DTDCompletionParser.class);
    private List<TagInfo> tagList = new CopyOnWriteArrayList<TagInfo>();
    ...
    protected TagInfo getTagInfo(String name)
    {
        ...
        {
            if (tagInfo.getTagName().equals(name))
            {
                return tagInfo;
            }
        }

        TagInfo info = new TagInfo(name, true);
        tagList.add(info);
                ...
                if (obj[i] instanceof DTDElement)
                {
                    ...
                    while (ite.hasNext())
                    {
                        ...
                        if (attrType instanceof DTDEnumeration)
                        {
                            ...
                            for (String dtdEnumItem : items)
                            {
                                attrInfo.addValue(dtdEnumItem);
                            }
                        }
                    }
